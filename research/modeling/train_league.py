# research/modeling/train_league.py
#
# Milestone 4: generalises train_epl.py to any configured league.
#
# Usage:
#   python train_league.py --league EPL --model dixon_coles   # default
...
#   outputs/championship_dixon_coles.json
#   outputs/championship_poisson.json
#
# To add a new league, add one entry to LEAGUE_REGISTRY below.
...
# ---------------------------------------------------------------------------
# League registry — the only section to edit when adding a new league
# ---------------------------------------------------------------------------

LEAGUE_REGISTRY: dict[str, dict[str, str]] = {
    "EPL": {
        "label":       "EPL",
        "input_csv":   "inputs/epl_matches.csv",
        "output_slug": "epl",
    },
    "Championship": {
        "label":       "Championship",
        "input_csv":   "inputs/championship_matches.csv",
        "output_slug": "championship",
    },
}

# ---------------------------------------------------------------------------
# Model / training constants (shared across all leagues)
# ---------------------------------------------------------------------------
...
LAMBDA_REG               = 0.1
RHO_DEGENERATE_THRESHOLD = 1e-3   # DC fits with |rho| below this are relabelled

ROOT = Path(__file__).parent


# ---------------------------------------------------------------------------
# Helpers  (identical to train_epl.py)
# ---------------------------------------------------------------------------
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Train a goal-scoring model on historical match data."
    )
    parser.add_argument(
        "--league",
        choices=list(LEAGUE_REGISTRY.keys()),
        default="EPL",
        help="League to train on (default: EPL).",
    )
    parser.add_argument(
        "--model",
    ...
    league_cfg     = LEAGUE_REGISTRY[args.league]
    target_league  = league_cfg["label"]
    input_csv      = ROOT / league_cfg["input_csv"]
    requested_model: str = args.model
    output_json    = ROOT / "outputs" / f"{league_cfg['output_slug']}_{requested_model}.json"

    # [1/7] Load
    if not input_csv.exists():
        print(f"ERROR: input CSV not found at {input_csv}", file=sys.stderr)
        print(
            "Run fetch_epl.py first:  python fetch_epl.py --league "
            + args.league,
            file=sys.stderr,
        )
        return 1
    ...
    if rows_used == 0:
        print(
            f"ERROR: no usable rows after filtering. "
            f"Check that the CSV contains rows with league='{target_league}'.",
            file=sys.stderr,
        )
        return 3
    ...
    if rows_used < 100:
        warnings.append(
            ...
            "Recommend at least 2 full seasons for stable estimates."
        )
    ...
    for t, c in sorted(
        (t, c) for t, c in train_team_counts.items() if c < MIN_MATCHES_PER_TEAM
    ):
        warnings.append(
    ...
    for m in holdout_matches:
        ...
        matrix, lam_h, lam_a = build_scoreline_matrix(
            params, m["home_team"], m["away_team"]
        )
