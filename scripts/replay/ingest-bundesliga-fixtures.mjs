...
const OUT_FILE = path.join(DATA_DIR, 'historical', 'bundesliga_2024_25_fixtures.json');
...
async function resolveALeague() {
  ...
  const match = leagues.find(l => {
    ...
    return (
      ...
      (l.country?.name === 'Germany')
    );
  });

  if (!match) {
    throw new Error(`Could not resolve bundesliga for Germany season ${SEASON}`);
  }

  return {
    leagueId: match.league.id,
    leagueName: `Germany - ${match.league.name}`,
  };
}
...
async function main() {
  ...
  const canonical = fixtures.map(f => {
    ...
    return {
      fixtureId: buildFixtureId('bundesliga', kickoffUtc, homeTeam, awayTeam),
