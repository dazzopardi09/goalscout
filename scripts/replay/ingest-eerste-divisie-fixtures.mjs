...
const OUT_FILE = path.join(DATA_DIR, 'historical', 'eerste_divisie_2024_25_fixtures.json');
...
async function resolveLeague() {
  ...
  const match = leagues.find(l => {
    ...
    return (
      country === 'Netherlands' &&
      (name.includes('eerste divisie') || name.includes('eerstedivisie'))
    );
  });

  if (!match) {
    throw new Error(`Could not resolve Eerste Divisie for Netherlands season ${SEASON}`);
  }
...
async function main() {
  const { leagueId, leagueName } = await resolveLeague();
  ...
  const canonical = fixtures.map(f => {
    ...
    return {
      fixtureId: buildFixtureId('eerste_divisie', kickoffUtc, homeTeam, awayTeam),
      leagueKey: 'eerste_divisie',
      leagueName,
      season: '2024-25',
      kickoffUtc,
      homeTeam,
